...
namespace CloudSeed
{
	...
	public:
			...
			, highShelf(AudioLib::Biquad::FilterType::HighShelf, samplerate)
		{
			...
			tempBuffer = new float[bufferSize]; // Flick: TODO - consider pool allocation
			mixedBuffer = new float[bufferSize]; // Flick: TODO - consider pool allocation
			filterOutputBuffer = new float[bufferSize]; // Flick: TODO - consider pool allocation

			lowShelf.Slope = 1.0f; // Flick: double→float
			lowShelf.SetGainDb(-20);
			lowShelf.Frequency = 20;

			highShelf.Slope = 1.0f; // Flick: double→float
			...
			SetDiffuserSeed(1, 0.0f); // Flick: double→float
		}
		...
		{
			diffuser.SetModulationEnabled(amount > 0.0f); // Flick: double→float
		...
		void ClearBuffers()
		{
			...
			{
				tempBuffer[i] = 0.0f; // Flick: double→float
				filterOutputBuffer[i] = 0.0f; // Flick: double→float
			}
		}
	};
}

...
// Flick: removed spurious self-include
#include "Utils.h"
#include "FastSin.h"
...
namespace CloudSeed
{
	...
	public:
		...
		ModulatedDelay(int bufferSize, int delayBufferSizeSamples, int sampleDelay)
		{
			...
			writeIndex = 0;
			modPhase = 0.01f + 0.98f * (std::rand() / (float)RAND_MAX); // Flick: float literals
			ModRate = 0.0f;   // Flick: float literal
			ModAmount = 0.0f; // Flick: float literal
			Update();
		}
	...
	private:
		void Update()
		{
			...
			if (modPhase > 1)
				modPhase = fmodf(modPhase, 1.0f); // Flick: fmodf + float literal
			...
			gainA = 1.0f - partial; // Flick: float literal
			gainB = partial;
